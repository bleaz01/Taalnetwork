...
const UpdateRegister = ({ id }) => {
	...
	const handleFocusTags = (item) => {
		if (focusTags.length < 5 && !focusTags.includes(item)) setFocusTags([ ...focusTags, item ]);
	};
				...
				<form onSubmit={handleSubmit(onSubmit)} className="flex flex-col justify-between">
					<div className="flex flex-col p-5">
						...
						<div>
							...
							<div>
								...
								<div className="flex justify-between w-full">
									<select
										{...register('whyLearn')}
										className=" pl-3 pr-3 py-3 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500"
									>
											...
											focusTags.map((item) => {
												return (
													<div className="flex justify-between m-1 mr-2 w-24 bg-baseColor text-whiteColor rounded-xl p-2 font-bold text-sm leading-loose cursor-pointer">
														...
														<span
															onClick={() =>
																setFocusTags(focusTags.filter((x) => x !== item))}
						...
						<div className="w-full px-3 mb-5">
							<button className="block w-full max-w-xs mx-auto bg-baseColor hover:bg-indigo-700 focus:bg-indigo-700 text-white rounded-lg px-3 py-3 font-semibold">
								SEND
							</button>
						</div>
					</div>
